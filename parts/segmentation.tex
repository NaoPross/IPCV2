...
\section{Image Segmentation\textbook{699-810}}
...
\subsection{Fundamentals\textbook{700}}
...
\end{itemize}
\subsubsection{Detection of Isolated Points\textbook{706}}
...
\subsubsection{Line Detection\textbook{707}}
...
\subsubsection{Edge Models\textbook{710}}
...
\subsubsection{Basic edge detection\textbook{716}}
...
\subsubsection{More Advanced Techniques for Edge Detection\textbook{724}}
\paragraph{Marr-Hildreth edge detector\textbook{724}}
...
\begin{figure}[h]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\adjustbox{scale=0.5}{\input{figures/tikz/segmentation/MarrHildreth}}
		\caption{Cross Section of the negative of the LoG, showing zero crossings}
	\end{subfigure}
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\adjustbox{scale=0.75}{\input{figures/tikz/segmentation/MarrHildrethMatrix}}
...
\paragraph{Canny edge detector\textbook{729}}
\begin{enumerate}
...
\subsubsection{Linking edge points\textbook{735}}
Three schemes to link edges:\\
\paragraph{Local processing\textbook{735}}
...
\begin{figure}[h]
	\centering
	\adjustbox{scale=0.5}{\input{figures/tikz/segmentation/RegionalProcessing}}
	\caption{Regional Processing}
\end{figure}
\paragraph{Global processing using the Hough transform\textbook{737}}
\begin{figure}[h]
	\centering
	\begin{subfigure}{0.3\textwidth}
		\input{figures/tikz/segmentation/HoughTransformXYPlane}
		\caption{$xy$-plane}
	\end{subfigure}
	\begin{subfigure}{0.3\textwidth}
		\input{figures/tikz/segmentation/HoughTransformRTPlane}
		\caption{$\rho\theta$-plane}
	\end{subfigure}
	\begin{subfigure}{0.3\textwidth}
		\input{figures/tikz/segmentation/HoughTransformAccumulatorCells}
...
\subsection{Thresholding\textbook{742}}
\subsubsection{Foundation\textbook{742}}
...
\begin{figure}[ht!]
	\begin{subfigure}{0.5\textwidth}
		\centering
		\input{figures/tikz/segmentation/threshold_single}
		\caption{single threshold}
	\end{subfigure}
	\begin{subfigure}{0.5\textwidth}
		\centering
		\input{figures/tikz/segmentation/threshold_multi}
...
\subsubsection{Basic global thresholding\textbook{746}}
...
\subsubsection{Optimum global thresholding using Otsu’s Method\textbook{747}}
...
\begin{flalign}
	...
	& & m_G: \text{ global mean}   && \label{eq:mG}\\
	\sigma_G^2 &= \sum_{i=0}^{L-1} (i-m_G)^2 p_i
	& & \sigma_G^2: \text{global variance}  && \label{eq:sigmaG}\\
	P_1(k) &=\sum_{i=0}^{k} p_i
	& &  P_1: \text{ probability of the background}    && \label{eq:P1}\\
	...
	m(k)&= \sum_{i=0}^{k}ip_i
	& &  m: \text{ cummulative mean up to level $k$}   && \label{eq:m} \\
	...
	\sigma_B^2(k)&=\frac{(m_G P_1(k) - m(k))^2}{P_1(k)(1-P_1(k))} & & \sigma_B^2 \text{ in dependence of } k && \label{eq:sigmaB} \\
	\sigma_B^2(k^*)&=\max\limits_{0\leq k \leq L-1} \sigma_B^2(k)
	& & \text{Optimum of k for maximizing is } k^*  && \\
	\eta(k)&=\frac{\sigma_B^2(k)}{\sigma_G^2}
	& & 0 \leq  \eta(k^*)\leq 1 \label{eq:eta}
\end{flalign}
...
\subsubsection{Using Image Smoothing to Improve Global Thresholding\textbook{752}}
...
\subsubsection{Using Edges to Improve Global Thresholding\textbook{753}}
...
\subsubsection{Multiple Thresholds\textbook{757}}
...
\end{flalign*}
\subsubsection{Variable Thresholding\textbook{761}}
...
\paragraph{Based on local image properties\textbook{761}}
...
\]
\paragraph{Using moving averages\textbook{763}}
...
\end{enumerate}
\subsubsection{Region splitting and merging\textbook{768}}
...
\begin{figure}[h]
	...
	\begin{subfigure}{0.3\textwidth}
		\centering
		\input{figures/tikz/segmentation/QuadtreePartionedImage}
		\caption{Partioned image}
	\end{subfigure}
	\begin{subfigure}{0.5\textwidth}
		\centering
		\input{figures/tikz/segmentation/QuadtreeTree}
...
\subsection{Segmentation using morphological watersheds\textbook{786}}
\subsubsection{Background\textbook{786}}
